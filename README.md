...
This repository is in pre-release development. The only current module is `memory`, which provides the typed in-process reference Compiler described below. No other Adapter module exists in this repository.

The compatible Weave core line has not been tagged yet, so the memory module is not independently resolvable with `GOWORK=off`. Development CI uses the public core revision pinned in its workflow; that pin must identify a revision containing the required `compilertest` package. Public module files contain no local `replace` directives.

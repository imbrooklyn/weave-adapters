...
- The `elasticsearch` Compiler as an independent Go 1.27 module, with the official go-elasticsearch v9.5.1 `*types.Query` / `types.QueryVariant` boundary, immutable Elasticsearch 9.5 Profile and explicit Mapping registry, exact scalar/cardinality/analyzed/nested/normalizer/null-marker capabilities, stable validate/emit passes, typed term/terms/range/exists/prefix/wildcard lowering, binary exists guards, all four Logic forms, constants, root Native and nestable Expr support, literal wildcard escaping, expensive-query policy intersections, fresh standard-query ownership, deterministic concurrent compilation, fuzz and benchmark coverage, real Elasticsearch 9.5.2/Lucene 10.5.1 match-set validation, and retained Elastic client/transport Apache-2.0 attribution.
...
- Development CI now includes all seven modules in the current-core workspace and runs independent `GOWORK=off` verify, tidy, test, race, vet, benchmark, and per-Adapter fuzz-smoke checks.

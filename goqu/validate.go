...
func (compiler Compiler) validateNode(
	node weave.NodeView[Expressions, Expression],
	depth int,
	root bool,
) (validatedNode, error) {
	if !node.Valid() {
...
func (compiler Compiler) validateComparison(
	...
	validated.identifier = metadata.identifier
...
func (compiler Compiler) validateNativeCondition(
	...
	expressions := view.Condition()
	if isNilLike(expressions) {
		return validatedNode{}, validationError(
			node,
			weave.CodeInvalidValue,
			0,
	...
	validated.native = ExpressionsOf(expressions...)
	return validated, nil
}
...
func assignableValue(value any, reported, expected reflect.Type) bool {
	...
	if _, expression := value.(exp.Expression); expression {
		return false
	}
